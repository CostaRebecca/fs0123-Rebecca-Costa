...
if (corrette >= 50) {
    ...
    const contenitore = document.getElementById('immagini');
    const larghezzaContenitore = contenitore.offsetWidth;
    const altezzaContenitore = contenitore.offsetHeight;

    let capra = [
        {
            src: 'GOAT/capraasolotesta.png',
            x: -100,
            y: 300
        },
        {
            src: 'GOAT/capratagliata.png',
            x: 200,
            y: -200
        },
        {
    ...
    img.style.left = Math.random() * larghezzaContenitore + 'px'; // posizione randomica sull'asse X
    img.style.top = Math.random() * altezzaContenitore + 'px'; // posizione randomica sull'asse Y
    ...
    /*for (let j = 0; j < 50; j++) { // ripete 3 volte
      for (let i = 0; i < capra.length; i++) {
        const img = document.createElement('img');
        img.src = capra[i].src;
        img.style.position = 'absolute';
        img.style.left = Math.random() * larghezzaContenitore + 'px'; // posizione randomica sull'asse X
